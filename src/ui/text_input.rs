//! 完整文本输入组件 - 支持退格删除、鼠标选择、剪贴板等
//! 基于 gpui examples/input.rs 实现

use gpui::{
    actions, App, Bounds, ClipboardItem, Context, CursorStyle, Element, ElementId, ElementInputHandler,
    Entity, EntityInputHandler, FocusHandle, Focusable, GlobalElementId, InteractiveElement, KeyBinding,
    LayoutId, MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, ParentElement, Pixels, Point,
    SharedString, ShapedLine, Style, Styled, TextRun, UTF16Selection, UnderlineStyle, Window, fill,
...
impl TextInput {
    ...
    pub fn set_content(&mut self, s: impl Into<SharedString>, cx: &mut Context<Self>) {
        self.content = s.into();
...
impl EntityInputHandler for TextInput {
    ...
    fn replace_text_in_range(
        ...
        self.content = (self.content[..range.start].to_owned() + new_text + &self.content[range.end..])
            .into();
...
pub fn key_bindings() -> Vec<KeyBinding> {
    vec![
        ...
        KeyBinding::new("delete", InputDelete, None),
        KeyBinding::new("left", InputLeft, None),
        KeyBinding::new("right", InputRight, None),
        KeyBinding::new("shift-left", InputSelectLeft, None),
        KeyBinding::new("shift-right", InputSelectRight, None),
        KeyBinding::new("cmd-a", InputSelectAll, None),
        KeyBinding::new("ctrl-a", InputSelectAll, None),
        KeyBinding::new("cmd-v", InputPaste, None),
        KeyBinding::new("ctrl-v", InputPaste, None),
        KeyBinding::new("cmd-c", InputCopy, None),
        KeyBinding::new("ctrl-c", InputCopy, None),
        KeyBinding::new("cmd-x", InputCut, None),
        KeyBinding::new("ctrl-x", InputCut, None),
        KeyBinding::new("home", InputHome, None),
        KeyBinding::new("end", InputEnd, None),
    ]
}
